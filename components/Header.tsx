...
export default function Header({ cartItems, cartTotal }: HeaderProps) {
  ...
  return (
    <header className="backdrop-blur-sm border-b border sticky top-0 z-50">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-20">
          {/* Logo Section */}
          <div className="flex items-center space-x-4">
            ...
            <div className="leading-tight">
              <h1 className="text-2xl font-extrabold text-gray-800 tracking-tight">EggFinity</h1>
              <p className="text-xs text-gray-500 font-medium">Your Unique Memory Game</p>
            </div>
          </div>
          ...
          <nav className="hidden md:flex items-center space-x-6">
            {navLinks.map(({ label, href }) => (
              <a
                key={label}
                href={href}
                className="text-sm font-medium text-gray-600 hover:text-[#f6e79e] transition-colors"
              >
                {label}
              </a>
            ))}
          ...
          <div className="flex items-center space-x-4">
            <button className="relative p-3 rounded-xl hover:bg-gray-100 transition-colors group">
              <ShoppingCart className="h-6 w-6 text-gray-600 group-hover:text-gray-800 transition-colors" />
              {cartItems > 0 && (
                <span className="absolute -top-1.5 -right-1.5 bg-[#f6e79e] text-gray-900 text-[10px] font-bold rounded-full h-5 w-5 flex items-center justify-center animate-pulse shadow">
                  {cartItems}
                </span>
              )}
            </button>
          ...
          <div className="md:hidden mt-2 space-y-2 pb-4 px-2 animate-slide-down">
            {navLinks.map(({ label, href }) => (
              <a
                key={label}
                href={href}
                className="block text-sm font-medium text-gray-700 hover:text-[#f6e79e]"
              >
                {label}
              </a>
            ))}
