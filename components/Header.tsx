...
function HeaderContent() {
          ...
          try {
            ...
            if (Array.isArray(cartItems) && cartItems.length > 0) {
              items = cartItems.reduce((sum, item) => {
                let count = 0;
                if (item.designs && Array.isArray(item.designs)) {
                  count = item.designs.length;
                } else if (item.designCount) {
                  count = item.designCount;
                } else if (item.count) {
                  count = item.count;
                }
                return sum + count;
  ...
  return (
    <header
      className={`backdrop-blur-md border-b border-gray-200/50 sticky top-0 z-50 transition-all duration-300 ${
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex items-center justify-between h-20">
          ...
          >
            ...
            <div className="leading-tight">
              <h1 className="text-lg sm:text-xl font-extrabold text-gray-800 group-hover:text-[#f6e79e] transition-colors">
                Eggception
              </h1>
          ...
          <nav className="hidden lg:flex items-center space-x-6">
            {navLinks.map(({ label, href }) => (
              <Link
                key={label}
                href={href}
                className="text-base font-medium hover:text-[#edd040] transition-colors relative group"
              >
                {label}
                <span className="absolute -bottom-1 left-0 w-0 h-0.5 bg-[#f6e79e] transition-all duration-300 group-hover:w-full"></span>
              </Link>
          ...
          <div className="flex items-center space-x-2 sm:space-x-3">
            ...
            <Link
              href="/account"
              className="hidden sm:flex items-center gap-1 sm:gap-2 hover:bg-white/20 px-2 sm:px-3 py-1.5 rounded-xl group"
            >
              <User className="h-5 w-5 text-gray-700 group-hover:text-[#f6e79e]" />
              <span className="text-sm sm:text-base font-medium text-gray-700 group-hover:text-[#f6e79e]">
                {t.nav.account}
              </span>
            </Link>
            ...
            <Link
              href="/checkout"
              className="relative flex items-center gap-1 sm:gap-2 hover:bg-white/20 px-2 sm:px-3 py-1.5 rounded-xl group"
            >
              <ShoppingCart className="h-5 w-5 text-gray-700 group-hover:text-[#f6e79e]" />
              <div className="hidden sm:flex flex-col items-start">
                <span className="text-xs text-gray-600">{t.nav.cart}</span>
                <span className="text-sm sm:text-base font-semibold text-gray-800">
                  {cartItems > 0
                    ? `${cartItems} ${t.nav.items}`
                    : t.nav.emptyCart}
                </span>
              </div>
              {cartItems > 0 && (
                <span className="absolute -top-1.5 -right-1.5 bg-[#f6e79e] text-gray-900 text-[10px] font-bold rounded-full h-5 w-5 sm:h-6 sm:w-6 flex items-center justify-center animate-pulse shadow-md">
                  {cartItems}
                </span>
              )}
            </Link>
            ...
            {/* PRICE */}
            {cartTotal > 0 && (
              <div className="hidden sm:block text-sm sm:text-base font-semibold text-gray-800 bg-white/30 px-2 sm:px-3 py-1.5 rounded-lg border border-gray-200/50">
                €{cartTotal.toFixed(2)}
              </div>
            )}
            ...
            <button
              onClick={switchLanguage}
              className="text-xs sm:text-sm font-semibold px-2 sm:px-3 py-1 border border-white/40 rounded-lg hover:bg-white/20 transition-all"
            >
              {lang === "en" ? t.nav.languages.de : t.nav.languages.en}
            </button>
        ...
        {/* MOBILE MENU */}
        {menuOpen && (
          <div className="lg:hidden mt-2 space-y-1 pb-4 px-2 bg-white/90 rounded-lg border border-gray-200/50">
            {navLinks.map(({ label, href }) => (
              <Link
                key={label}
                href={href}
                onClick={() => setMenuOpen(false)}
                className="block text-sm sm:text-base font-medium text-gray-700 hover:text-[#f6e79e] py-2 px-3 rounded-lg hover:bg-white/20"
              >
                {label}
              </Link>
            ))}
            <div className="border-t border-gray-200/50 mt-2 pt-2">
              ...
              <div className="px-3 py-2 space-y-1">
                ...
                {cartTotal > 0 && (
                  <div className="flex justify-between text-sm sm:text-base">
                    <span className="text-gray-600">{t.nav.total}</span>
                    <span className="font-semibold text-[#f6e79e]">
                      €{cartTotal.toFixed(2)}
                    </span>
                  </div>
                )}
